...
- Regenerating `wiki/core/` against pinned sources → `ac_update_wiki`.
- API gate / version baseline → [`skills/ac_audit_skill_apis.md`](../skills/ac_audit_skill_apis.md).
...
**PyAutoCTI-specific content** (regenerated per domain, never copied blind): every
`ac_*` skill body, `wiki/core/` reference pages, the entire `wiki/literature/` sub-wiki,
