...
For users *learning* PyAutoCTI — students, workshop attendees, anyone new to the stack or
to lensing. The goal is that the user understands the workflow, not just gets a result.
...
- Point to PyAutoCTI workspace examples, the RTD / HowToLens docs, and `wiki/` pages.
...
- When a fit starts, tour the output folder rather than leaving the user watching a
  ...
  workspace's `__Output Folder__` / `__On The Fly Outputs__` prose, and what to open
  first.
...
`profile.md`: an expert who asks to be taught gets the *workflow* explained, not lensing from
first principles.

## What triggers inference

"I'm new to PyAutoCTI, how do I model this image?", "Teach me how SLaM works", "What example
should I read first?", "Explain what this source reconstruction means."
