...
- A new `skills/ac_<name>.md` file authored against `skills/_style.md`.
...
- Whether they have an existing dataset to validate the skill on, or want a synthetic
  one (in which case the new skill may chain into `ac_simulate_dataset`).
...
- New profile / model object → `PyAutoGalaxy` + `PyAutoCTI`.
...
- Existing usage patterns in `<Project>/test_<package>/` (tests) or
  `autocti_workspace/scripts/` (tutorials).
...
Save the new skill at `skills/ac_<task>.md`. Follow `_style.md`:
...
```bash
bash "${PYAUTO_BRAIN:-organs/PyAutoBrain}/bin/install.sh" --write-project-discovery autocti_assistant
bash "${PYAUTO_BRAIN:-organs/PyAutoBrain}/bin/install.sh" --check-project-discovery autocti_assistant
```
...
- The request is essentially "run this existing tutorial script for me" — point the
  user at the existing script in `autocti_workspace` and the relevant wiki page, but
