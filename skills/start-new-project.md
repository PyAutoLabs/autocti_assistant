...
> **`autocti_assistant` is the copilot; a science project is a separate repo.** This repo is
...
> this `autocti_assistant` clone** (cloning it on demand if absent), so there's one source of
...
> `autocti_workspace` example, `PyAutoReduce`'s scripts/docs, and the `--sample`/`--dataset`
...
### 4. Modeling scripts
> **Modeling scripts?** They live in `scripts/`, normally adapted from `autocti_workspace`.
...
This is a science project created with autocti_assistant. The assistant is the copilot; this
...
[autocti_assistant](https://github.com/PyAutoLabs/autocti_assistant), the PyAutoCTI AI
...
1. **Dated journal** → `wiki/project/YYYY-MM-DD-<slug>.md` (the `_template.md` shape:
   ...
   |-----|---------|------|-----------|----------|
   | smooth SLaM baseline | `python scripts/imaging.py --dataset slacs0946 --seed 42` | 42 | `output/slacs/slacs0946/mass_total/<hash>/` | autocti `<v>`, autofit `<v>`, assistant `<sha>` |
