...
description: Prepare a scoped update to autocti_assistant and propose it back upstream. Use this skill when the user says "contribute this back", "open a PR upstream", "suggest this to the assistant", or "send this to PyAutoLabs/autocti_assistant". It validates the git remote layout, confirms PR scope, creates or reuses a feature branch, stages only the intended files, commits with the repo's commit conventions, pushes either to the upstream repo (for collaborators) or the user's fork, and opens a draft PR against PyAutoLabs/autocti_assistant.
user-invocable: true
---
...
`PyAutoLabs/autocti_assistant`, whether the user is working from a personal fork or from a
...
Confirm the current repository is `autocti_assistant` and inspect its remotes:
...
- **Upstream target** — the remote that points to `PyAutoLabs/autocti_assistant`.
...
- `origin` = `PyAutoLabs/autocti_assistant`
...
Target `PyAutoLabs/autocti_assistant` and its default branch.
...
```bash
gh pr create \
  --repo PyAutoLabs/autocti_assistant \
  --base main \
...
When the branch already lives on `PyAutoLabs/autocti_assistant`, `--head <branch-name>` is
sufficient.
