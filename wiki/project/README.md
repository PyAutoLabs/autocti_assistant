...
The memory for *this clone* of `autocti_assistant`, and for any science project
...
- `profile.md` — **who you are working with**: background, interaction mode, HPC access
  ...
  science goal is `state.md`'s, not this file's. Created on demand from
  [`_profile_template.md`](./_profile_template.md) — it does **not** exist until a user
  volunteers something durable, so it is named here without a link.
