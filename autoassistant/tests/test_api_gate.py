...
runs under (source `activate.sh` first). Each case imports autocti, so the file is slow
...
GOOD = "import autocti.plot as aplt; aplt.subplot_fit_ci(fit=fit)"
...
def test_validator_file_mode(tmp_path):
    ...
    script.write_text(f"import autocti as al\n{STALE_PLOTTER}\n", encoding="utf-8")
...
def test_hook_allows_non_python_command_without_import():
    # Pre-screen must short-circuit (no autocti import) — assert both allow and speed.
...
def test_hook_validates_py_file_argument(tmp_path):
    ...
    good.write_text(f"import autocti.plot as aplt\n{GOOD}\n", encoding="utf-8")
    assert _decision(_run_hook(f"{sys.executable} {good}")) is None

    bad = tmp_path / "bad.py"
    bad.write_text(f"import autocti as al\n{STALE_PLOTTER}\n", encoding="utf-8")
...
def test_validate_source_reports_broken_stack_once_not_stale():
    ...
    mod._module_cache["autocti"] = None
    mod._import_errors["autocti"] = wall
    ...
    assert list(import_failed) == ["autocti"]  # reported once, not per symbol
    assert len(import_failed["autocti"]) <= 201  # truncated, not the full wall


def test_render_installation_check_groups_identicac_errors():
    mod = _load_validator_module()
    check = mod.InstallationCheck(
        status="import_failed", python="py", prefix="env",
        versions={}, locations={}, missing=[],
        errors={name: "Boom: identical wall" for name in ("autofit", "autogalaxy", "autocti")},
    ...
    assert "autofit, autogalaxy, autocti import failed" in text
