...
DEFAULT_BIBLIOGRAPHY = (
    ROOT / "wiki" / "literature" / "bibliography" / "autocti_literature.bib"
)
DEFAULT_ALIASES = (
...
def extract_canonicac_keys(markdown: str, path: Path = Path("<memory>")) -> tuple[SourceCitation, ...]:
...
def collect_source_citations(sources_dir: Path) -> tuple[SourceCitation, ...]:
    ...
    for path in sorted(sources_dir.glob("*.md")):
        citations.extend(
            extract_canonicac_keys(path.read_text(encoding="utf-8"), path=path)
        )
