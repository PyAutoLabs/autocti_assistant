...
This is the mechanical half of the `ac_audit_skill_apis` skill. The skill drives
...
# ---------------------------------------------------------------------------
# API baseline. autocti_assistant is tied to an autocti version; this file
...
# length-descending so the regex alternation prefers `autocti` over `al`.
# ---------------------------------------------------------------------------
...
DENY_LIST: tuple[IdiomRule, ...] = (
    IdiomRule(
        ...
        replacement=(
            ...
            "result_list = search.fit(model=factor_graph.globac_prior_model, "
            "analysis=factor_graph)"
        ),
        citation=(
            "PyAutoFit:autofit/graphical/declarative/factor/analysis.py; "
            "autocti_workspace:scripts/multi/start_here.py"
        ),
    ),
    IdiomRule(
...
def select_files(root: Path, scope: str) -> list[Path]:
    ...
    # recursively because scripts/ mirrors autocti_workspace's nested layout.
...
def _installation_kind(module_locations: dict[str, str]) -> str:
    ...
    try:
        distribution = importlib.metadata.distribution("autocti")
    ...
    location = module_locations.get("autocti", "")
...
def render_installation_check(check: InstallationCheck) -> str:
    ...
    if check.locations.get("autocti"):
        lines.append(f"  autocti: {check.locations['autocti']}")
    ...
    # autogalaxy and autocti all fail with the same multi-paragraph message —
    ...
    if check.status != "ready":
        lines.extend(
            [
                ...
                "  Otherwise use `skills/ac_setup_environment.md`; do not install into an",
...
def compute_baseline() -> dict:
    ...
    return {
        ...
        "and skills/ac_audit_skill_apis.md. Regenerate with --write-baseline.",
...
def check_version(root: Path) -> int:
    ...
    if not version_drift and not hash_drift:
        print(
            ...
            f"(autocti {current['versions']['autocti']}, generated {baseline.get('generated')})."
        )
        return 0
    ...
    print(
        ...
        "pinned version (`pip install -U autocti`), or run `--scope all` to audit drift "
...
def run_code_gate(*, code: str | None, file: str | None) -> int:
    ...
    if import_failed:
        ...
        print(
            "  Fix the environment first (skills/ac_setup_environment.md); if this is "
